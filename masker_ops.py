...
class MaskRCNN(nn.Module):
    def __init__(self,k,nms_threshold,img_h,img_w,cf_h,cf_w,mh,mw,device=None,visualize=False,train_rpn=True):
        ...
        if train_rpn:
            self.proposal_filter.apply(tl.freeze_params)
            self.roi_align.apply(tl.freeze_params)
            self.mask_generator.apply(tl.freeze_params)
            self.whole_mask.apply(tl.freeze_params)
        else:
            self.rpn.apply(tl.freeze_params)
