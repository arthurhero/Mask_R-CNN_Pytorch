'''
Mask R-CNN using Feature Pyramid Networks (FPN)
It takes in an image and generates K object masks (each mask is of img size)
...
masker_ckpt_path='logs/masker_rpn.ckpt'
...
def train_masker():
    ...
    #modules
    mask_rcnn=mo.MaskRCNN(k,nms_threshold,img_size,img_size,fh,fw,mh,mw,device,visualize,train_rpn).to(device)
    mask_rcnn.train()
            ...
            if train_rpn:
                ...
                if step%10==0:
                    print('Epoch [{}/{}] , Step {}, Loss: {:.4f}, l_cls: {:.4f}, l_bbox: {:.4f}'
                                .format(e+1,epoch,step,loss.item(),l_cls.item(),l_bbox.item()))
            else:
